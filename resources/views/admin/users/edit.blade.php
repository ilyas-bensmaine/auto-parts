...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{__('Editer un utilisateur')}}</h1>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    </section>
              ...
              <div class="card-body">
                <form method="POST" action="{{ route('admin.users.update', ['language'=> app()->getlocale(), 'user'=>$user->id]) }}">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <input id="confirm-password" type="password" class="form-control" name="confirm-password" autocomplete="new-password">
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                          <select name="roles" class="select2  @error('roles') is-invalid @enderror" data-placeholder="{{__('Sélectionnez un rôle')}}" style="width: 100%;">
                            @foreach ($roles as $role)
                              <option @if ($userRole == $role) selected @endif >{{$role}}</option>
                            @endforeach
                          </select>
                          @error('roles')
                          <span class="invalid-feedback" role="alert">
